...
console.log('\n── 2. Ruta "Empiezo de cero" — ordena sin ocultar ──');
{
  ...
  const fuera = resto(d);
  is(fuera.length === 23, `los otros 23 siguen presentes (vi ${fuera.length})`);
  ...
  is(/Los otros 23 quedan abajo/.test(d.getElementById('ctx-t').textContent), 'el texto avisa que el resto sigue abajo');
}
...
console.log('\n── 3. Ruta "Reemplazar mi agencia" — arranca por el prerequisito ──');
{
  ...
  is(ids(ruta).join() === 'mcp,skills22,anuncios3,paidmedia,masterprompt,clonar',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 22, 'y los otros 22 quedan abajo');
}
...
console.log('\n── 4. Ruta "Conectar a mis cuentas" ──');
{
  ...
  is(ids(ruta).join() === 'mcp,tutorial,googleads,masterprompt,paidmedia,docsmeta',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 22, 'los otros 22 abajo');
}
...
console.log('\n── 6. Estado: visitado, avance y prerequisito cumplido ──');
{
  ...
  is(d.getElementById('tk-bar').style.width === (1/28*100) + '%', 'barra al ' + (100/28).toFixed(1) + '%');
