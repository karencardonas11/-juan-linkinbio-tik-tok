...
console.log('\n── 2. Ruta "Empiezo de cero" — ordena sin ocultar ──');
{
  ...
  const fuera = resto(d);
  is(fuera.length === 11, `los otros 11 siguen presentes (vi ${fuera.length})`);
  ...
  is(/Los otros 11 quedan abajo/.test(d.getElementById('ctx-t').textContent), 'el texto avisa que el resto sigue abajo');
}
...
console.log('\n── 3. Ruta "Reemplazar mi agencia" — arranca por el prerequisito ──');
{
  ...
  is(ids(ruta).join() === 'mcp,skills22,anuncios3,paidmedia,masterprompt,clonar',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 10, 'y los otros 10 quedan abajo');
}
...
console.log('\n── 4. Ruta "Conectar a mis cuentas" ──');
{
  ...
  is(ids(ruta).join() === 'mcp,tutorial,googleads,masterprompt,paidmedia,docsmeta',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 10, 'los otros 10 abajo');
}
...
console.log('\n── 6. Estado: visitado, avance y prerequisito cumplido ──');
{
  ...
  is(d.getElementById('tk-bar').style.width === (1/16*100) + '%', 'barra al ' + (100/16).toFixed(1) + '%');
