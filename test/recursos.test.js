...
console.log('\n── 2. Ruta "Empiezo de cero" — ordena sin ocultar ──');
{
  ...
  const fuera = resto(d);
  is(fuera.length === 13, `los otros 13 siguen presentes (vi ${fuera.length})`);
  ...
  is(/Los otros 13 quedan abajo/.test(d.getElementById('ctx-t').textContent), 'el texto avisa que el resto sigue abajo');
}
...
console.log('\n── 3. Ruta "Reemplazar mi agencia" — arranca por el prerequisito ──');
{
  ...
  is(ids(ruta).join() === 'mcp,skills22,anuncios3,paidmedia,masterprompt,clonar',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 12, 'y los otros 12 quedan abajo');
}
...
console.log('\n── 4. Ruta "Conectar a mis cuentas" ──');
{
  ...
  is(ids(ruta).join() === 'mcp,tutorial,googleads,masterprompt,paidmedia,docsmeta',
     'orden: ' + ids(ruta).join(' → '));
  is(resto(d).length === 12, 'los otros 12 abajo');
}
...
console.log('\n── 6. Estado: visitado, avance y prerequisito cumplido ──');
{
  ...
  is(d.getElementById('tk-bar').style.width === (1/18*100) + '%', 'barra al ' + (100/18).toFixed(1) + '%');
